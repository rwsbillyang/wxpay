...
import com.github.rwsbillyang.wxpay.protocol.RefundQueryReqData;
...
public class RefundQueryBusiness {
    ...
    public interface ResultListener{
        ...
        void onFailByReturnCodeError(ResponseMap refundQueryResData);

        //API返回ReturnCode为FAIL，支付API系统返回失败，请检测Post给API的数据是否规范合法
        void onFailByReturnCodeFail(ResponseMap refundQueryResData);

        //支付请求API返回的数据签名验证失败，有可能数据被篡改了
        void onFailBySignInvalid(ResponseMap refundQueryResData);

        //退款查询失败
        void onRefundQueryFail(ResponseMap refundQueryResData);

        //退款查询成功
        void onRefundQuerySuccess(ResponseMap refundQueryResData);
  ...
  //  private RefundQueryService refundQueryService;
    ...
    public void run(RefundQueryReqData refundQueryReqData,RefundQueryBusiness.ResultListener resultListener) throws Exception {
       ...
       // refundQueryServiceResponseString = refundQueryService.request(refundQueryReqData);
        refundQueryServiceResponseString = WXPayManager.getInstance().sendRequest(refundQueryReqData);
        ...
        ResponseMap refundQueryResData =new ResponseMap(refundQueryServiceResponseString);

        ReportReqData reportReqData = new ReportReqData(
                refundQueryReqData.getDevice_info(),
                WXPayManager.REQUEST_API_ARRAY[WXPayManager.TYPE_INDEX_REFUND_QUERY],
        ...
        if (refundQueryResData.getReturn_code().equals("FAIL")) {
           ...
           // if (!Signature.checkIsSignValidFromResponseString(refundQueryServiceResponseString,WxPayConfigsCache.getMchSecretKey())) {
            ...
            if (refundQueryResData.getResult_code().equals("FAIL")) {
                log.info("出错，错误码：" + refundQueryResData.getErr_code() + "     错误信息：" + refundQueryResData.getErr_code_des());
