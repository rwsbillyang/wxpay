...
import com.github.rwsbillyang.wxpay.protocol.OrderQueryReqData;
import com.github.rwsbillyang.wxpay.protocol.ReportReqData;
import com.github.rwsbillyang.wxpay.protocol.ResponseMap;
import com.github.rwsbillyang.wxpay.protocol.ReverseReqData;
import com.github.rwsbillyang.wxpay.protocol.ScanPayReqData;
...
public class ScanPayBusiness {
    ...
    public interface ResultListener {
        ...
        void onFailByReturnCodeError(ResponseMap scanPayResData);

        //API返回ReturnCode为FAIL，支付API系统返回失败，请检测Post给API的数据是否规范合法
        void onFailByReturnCodeFail(ResponseMap scanPayResData);

        //支付请求API返回的数据签名验证失败，有可能数据被篡改了
        void onFailBySignInvalid(ResponseMap scanPayResData);


        //用户用来支付的二维码已经过期，提示收银员重新扫一下用户微信“刷卡”里面的二维码
        void onFailByAuthCodeExpire(ResponseMap scanPayResData);

        //授权码无效，提示用户刷新一维码/二维码，之后重新扫码支付"
        void onFailByAuthCodeInvalid(ResponseMap scanPayResData);

        //用户余额不足，换其他卡支付或是用现金支付
        void onFailByMoneyNotEnough(ResponseMap scanPayResData);

        //支付失败
        void onFail(ResponseMap scanPayResData);

        //支付成功
        void onSuccess(ResponseMap scanPayResData);
   ...
   // private ReverseService reverseService;
    ...
    public void run(ScanPayReqData scanPayReqData, ResultListener resultListener) throws Exception {
       ...
       // payServiceResponseString = scanPayService.request(scanPayReqData);
        payServiceResponseString=WXPayManager.getInstance().sendRequest(scanPayReqData);
        ...
        ResponseMap scanPayResData = new ResponseMap(payServiceResponseString);
        ...
        ReportReqData reportReqData = new ReportReqData(
                ...
                WXPayManager.REQUEST_API_ARRAY[WXPayManager.TYPE_INDEX_MICROPAY],
    ...
    private boolean doOnePayQuery(String outTradeNo) throws Exception {
        ...
        OrderQueryReqData scanPayQueryReqData = new OrderQueryReqData("",outTradeNo);
      //  payQueryServiceResponseString = orderQueryService.request(scanPayQueryReqData);
        payQueryServiceResponseString = WXPayManager.getInstance().sendRequest(scanPayQueryReqData);
        
        log.info("支付订单查询API返回的数据如下：");
        log.info(payQueryServiceResponseString);

        //将从API返回的XML数据映射到Java对象
        ResponseMap orderQueryResData = new ResponseMap(payQueryServiceResponseString);
    ...
    private boolean doOneReverse(String outTradeNo) throws Exception {
       ...
       // reverseResponseString = reverseService.request(reverseReqData);
        reverseResponseString = WXPayManager.getInstance().sendRequest(reverseReqData);
        
        
        log.info("撤销API返回的数据如下：");
        log.info(reverseResponseString);
        //将从API返回的XML数据映射到Java对象
        ResponseMap reverseResData = new ResponseMap(reverseResponseString);
